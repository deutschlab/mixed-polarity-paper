...
from config import (
    FULL_RECI_CONNECTIONS_FTR, NEURON_TABLE_FTR, OUTPUT_DIR, METHODS_DIR,
)
...
_out_dir = OUTPUT_DIR / "fig5" / "syn_type_x_reci_x_dominance"
...
final_df=final_df.merge(nodesG[['root_id','super_class']],left_on='Node A',right_on='root_id',how='left')
final_df=final_df.merge(nodesG[['root_id','super_class']],left_on='Node B',right_on='root_id',how='left')
#%%
final_df=final_df.dropna(subset=['primary_type_A','primary_type_B'])
#%%
...
for type_1 in ['AD','DA','AA','DD']:
   for type_2 in ['AD','DA','AA','DD']: 
        ...
        if type_1==type_2:
            ...
            c1a=final_df[(final_df[f'node_A_pre_{type_1}']>final_df['dim_A->B']/2)&(final_df[f'node_B_pre_{type_2}']>final_df['dim_B->A']/2)][[f'node_A_pre_{type_1}','dim_A->B',f'node_B_pre_{type_2}','dim_B->A','same_type']]
            c1b=final_df[(final_df[f'node_B_pre_{type_1}']>final_df['dim_B->A']/2)&(final_df[f'node_A_pre_{type_2}']>final_df['dim_A->B']/2)][[f'node_B_pre_{type_1}','dim_B->A',f'node_A_pre_{type_2}','dim_A->B','same_type']]
            
            c1a.columns=[f'{type_1}',f'{type_1}->{type_2}_dim','f{type_2}',f'{type_2}->{type_1}_dim','same_type']
            c1b.columns=[f'{type_1}',f'{type_1}->{type_2}_dim','f{type_2}',f'{type_2}->{type_1}_dim','same_type']
            c1=pd.concat([c1a,c1b])
            
            c1_same=c1[c1['same_type']==1]
            c1_not_same=c1[c1['same_type']==0]
            
            total_same=len(c1_same)
            total_not_same=len(c1_not_same)
            prop_same=total_same/len(final_df[final_df['same_type']==1])
            prop_not_same=total_not_same/len(final_df[final_df['same_type']==0])
            
            d_non_diagonal[str(type_1)+'_'+str(type_2)]=[total_same,total_not_same,prop_same,prop_not_same]
...
for type_ in ['AD','DA','AA','DD']:
    ...
    ][[f'node_A_pre_{type_}', 'dim_A->B', f'node_B_pre_{type_}', 'dim_B->A', 'same_type']]
    
        
    df_same = df_v[df_v['same_type'] == 1]
    df_not_same = df_v[df_v['same_type'] == 0]
    
    total_same = len(df_same)
    total_not_same = len(df_not_same)
    ...
    d_diagonal[type_]=[total_same,total_not_same,prop_same,prop_not_same]
#%%# Define types
# Define types
types = ['AD', 'DA', 'AA', 'DD']
...
# For TOTALS
plot_heatmap_lower_with_diag(same_total, 'Same Type Total', same_total_lower_sum)
plot_heatmap_lower_with_diag(not_same_total, 'Not Same Type Total', not_same_total_lower_sum)
#%%
# For PERCENTAGES
plot_heatmap_lower_with_diag(same_prop, 'Same Type Percentage', same_total_lower_sum)
plot_heatmap_lower_with_diag(not_same_prop, 'Not Same Type Percentage', not_same_total_lower_sum)
...
sns.heatmap(
    ...
    cbar=True,
    cbar_kws={"location": "left"},
    ax=ax,vmin=0,vmax=50,    annot_kws={"size": 4}
